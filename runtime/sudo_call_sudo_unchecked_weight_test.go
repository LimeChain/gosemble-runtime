...
func Test_Sudo_SudoUncheckedWeight_Success(t *testing.T) {
	...
	weight := ctypes.Weight{
		RefTime:   ctypes.NewUCompactFromUInt(1),
		ProofSize: ctypes.NewUCompactFromUInt(1),
	}
	...
	call, err := ctypes.NewCall(metadata, "Sudo.sudo_unchecked_weight", callArg, weight)
	assert.NoError(t, err)
