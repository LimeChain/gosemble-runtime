...
func Test_Balances_UpgradeAccounts_NoUpgrades_Success(t *testing.T) {
	...
	call, err := ctypes.NewCall(metadata, "Balances.upgrade_accounts", []ctypes.AccountID{bob.AsID})
	assert.NoError(t, err)
	...
	assert.Nil(t, bytesStorageBob)

	expectedAliceFreeBalance := big.NewInt(0).Sub(
		balance, queryInfo.PartialFee.ToBigInt())
